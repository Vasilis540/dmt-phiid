...
Moved from the main text (Table 6 of the drafts of 21–22 September) on 23 September 2026. ts_gsr; nats; DMT pre-injection means; DiD with its inverted sign-flip 95 % interval (B21) and exact p; W = 60 except the last column, the global fit; the per-subject correlations with their Fisher-z intervals (B21 (d)). Source: `notes/review_results/partB/lag_tables.md`, `inference_rows_lag.csv`. The relative size of the contrast at each lag, the overlap of the autocorrelation contrasts' intervals and the spectral centroid are in S3 Text, section 9.
...
Moved from the main text (Table 8 of the drafts of 21–22 September) on 23 September 2026. The sts difference the estimator returns between two simulated conditions of equal analytic sts and different covariance, as a share of the observed contrast (−0.0809), by fitting length; the difference is estimated over 4,000 independent windows at W = 30, 60 and 840 (± is the standard error over those windows where the difference is not distinguishable from zero). The four matched pairs are described in S3 Text, section 6 (`notes/rev_sts_matched_null.py`; review of 14 September, section 5; a post hoc computation).
...
| # | group | quantity | set | field | mean | exact p (committed) | percentile interval (committed) | inverted interval | t interval | width ratio | zero in pct / inv / t | neg/14 | quoted in the text of 23 September 2026 (file:line at 90690f4) |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
0.8174 is the continuous-band value of the flat-spectrum r₁; S11 Table's 0.8176 is the same quantity on the 840-sample frequency grid.
...
Source: `manuscript/analysis_record.md` (the pre-registered section and the pre-run and outcome entries named in each row), `manuscript/prespecification_summary.md`, `notes/partB_prespec_2026-09-14.md` and `notes/partB4_diagnostic.md`. Part A lists, in date order, every prediction the record states before the computation it concerns, with the outcome and the verdict; where the outcome entry gives its own verdict that verdict is used, and where it gives none the prediction's stated criterion is applied. A prediction stated as a range is met only if the outcome lies inside it. Entries that recorded a reading rule or an outcome mapping without predicting which branch would obtain are listed with the branch that obtained and are not counted as predictions; so are the values the pre-run entries of 23 September 2026 mark as already known from the verification of 22 September (B21 (b) and (c); the reproductions in B23), which the outcome entries evaluate but call reproductions rather than predictions. Part B lists the post hoc computations the paper quotes, with a pre-run entry only where one is named. Labels B1–B24, B16b and B17b are the record's (S3 Text).

### Part A. Recorded predictions and their outcomes

| record entry (date) | prediction as recorded | outcome | verdict | reported at |
|---|---|---|---|---|
| Pre-registered analysis choices, the window-length decision tree (18ad8b4, 12 Sep 2026) | The analytic log-det correction "changes the differential bias by exactly zero", shifting every condition's sts by the same constant, −0.5·[ψ((N−3)/2) − ψ((N−4)/2)] ≈ −0.02 nats at N = 29, so the W = 30 criterion fails on its second clause and the fallback, W = 60 uncorrected, is the branch | Constants −0.02040 / −0.00917 / −0.00060 at N = 29 / 59 / 839, equal to the recorded formula to 10⁻¹²; the differential bias changed by exactly zero (max 10⁻⁶, the tables' rounding); both clauses failed and W = 60 became the primary window (step 2, 13 Sep 2026) | met | not quoted; its result, W = 60 as the primary window, is in S1 Text and Methods, Estimator |
| Pre-registered analysis choices, the noise-floor prediction (18ad8b4, 12 Sep 2026) | The adjacent-pair (tier-1) criterion "will be marginal or fail, with the smallest pairs (late decay, bins ~20–28) near chance and the early decay pairs correctly signed" | "Half right": the early pairs were correctly signed (P ≥ 0.95); the late pairs were not near chance through noise but, at 2,000 runs, two per condition were wrong in expectation, which the 20,000-run repeat showed to be sampling error | partly met (the outcome entry's "half right") | not quoted (the tiers are defined in S1 Text) |
| Pre-registered analysis choices, the prediction for the 20,000-run repeat (13 Sep 2026) | Coupling S_analytic at W = 60 "≤ 0.89 and plausibly below 0.80", two pairs wrong in expectation; tier 1 fails | Every decay pair had the correct sign in expectation and coupling S rose to 0.923; "the prediction recorded before the repeat was wrong" | missed | not quoted |
...
| B14, the family-predicted atoms (20 Sep 2026, 19:07 UTC) | The family with unequal coefficients gives sts − (xtx + yty) < 0 (about −0.07 for the window-level scatter of a), rts = str below xtx, yty, and mirror atoms ≈ −rts; the sts residual equals the diagnostic's by construction | Excess −0.0844 observed, −0.1003 family-predicted (sign matches; the "about −0.07" an estimate); the ordering holds; mirrors ≈ −rts; residual identical to the diagnostic's: "prediction met on both variants" | met | Results 1; Table 1; S3 Text §1; Fig 2 |
| B15, the directed cross-lag component (20 Sep 2026, 19:07 UTC) | Antisymmetric deviation lowers sts (closed form: −0.0126 at ±0.05); whether the data's δ_anti accounts for the unlocated part of the run-level residual is the question, with no branch labels | The directed response is negative in 14 of 14 (−0.0064 [−0.0070, −0.0058] on ts_gsr) and, with the symmetric response −0.0071, gives the run-level residual −0.0137: about half directed and half symmetric | met | S3 Text §6 |
| B16, prewhitening (20 Sep 2026, 19:07 UTC) | On the family prewhitening removes S, so MMI-sts falls to the order of the cross-lag structure (near zero) and the contrast shrinks to the order of the cross-lag contrast | sts 0.22 after AR(p) whitening, a fifth of the raw level, and its contrast −0.026, an order of magnitude above the cross-lag contrasts, because the whitening is incomplete (r₁ 0.26 left): "prediction not met as recorded" | missed | Results 7; S3 Text §8; S11 Table |
| B17 (i), the calibration on AR(1) pairs, a pure autocorrelation change (20 Sep 2026, 19:07 UTC) | The residual ≈ its finite-sample expectation, +0.004 to +0.008 at W = 60; near zero at the global fit | W = 60 +0.0049 ± 0.0017, inside the band; the global fit's period-level residual +0.0089 ± 0.0028, "not the 'near zero' the entry recorded" | partly met (the outcome entry: "met"; the global-fit part missed) | Results 4; Table 3; S3 Text §7; S10 Table |
| B17 (ii), the calibration on AR(1) pairs, a coupling change | Residual ≈ −1.77 Δc at the global fit (first order), smaller at W = 60; δ_sym ≈ 0.94 Δc | Residual −0.0010, −0.0052, −0.0109, −0.0055 at Δc = +0.01, +0.02, +0.03, −0.02: negative for both signs and growing as Δc², "wrong in form"; δ_sym ≈ 0.74 Δc | missed (met in sign for positive Δc only) | S3 Text §7; S10 Table; Methods, Pre-registration and deviations |
| B17 (iii), the calibration on AR(1) pairs, both changes | Additive to first order | +0.0009 ± 0.0016 against +0.0049 − 0.0052: additive | met | S3 Text §7; S10 Table |
| B17 (iv), the calibration on AR(1) pairs, within-pair asymmetry | sts level lowered by the asymmetry, the residual near its expectation | Level 0.7131 against 0.7152 at W = 60 (far less than the closed form's population difference), residual +0.0051 ± 0.0018: "met" | met (the level fell by far less than the population difference; S10 Table) | S3 Text §7; S10 Table; Limitations |
| B18, the CCS increase decomposed (20 Sep 2026, 19:07 UTC) | "Prediction recorded: none (descriptive)" | The global-fit increase sits in the co-information term (+0.0174 of +0.0197) | no prediction | Results 6; S3 Text §5 |
| B19 (b), the within-window regression (20 Sep 2026, 19:07 UTC) | R² close to 0.80, "with \|a_x − a_y\| carrying the difference between 0.55 and 0.80" | R² = 0.808 against r² = 0.462 for r₁ alone; standardised coefficients r₁ +0.66, \|q\| −0.46, \|a_x − a_y\| −0.43: the outcome entry logged "prediction met"; the review of 24 Sep 2026 reads the second clause as unmet, since \|q\| carries as much of the difference as the asymmetry (given side by side, as the record and the review have them) | partly met (met in the outcome entry; the review's reading of the second clause) | Results 1; S3 Text §4 |
| B19 (c), the cross-half correlation | Below 0.953 and near the ceiling 0.729 | +0.742 and +0.645, mean +0.694 against 0.729: "met" | met | Results 2; S3 Text §5; Fig 3 |
| B19 (d), BCa intervals | BCa intervals wider at N = 14 | No BCa interval differed from its percentile interval by more than 10 % of the width (largest ratio 1.04): "did not materialise" | missed | S3 Text §5; S17 Table |
| B20, the regional map with regional r₁ partialled out (20 Sep 2026, 19:07 UTC) | The residual map's sensory–association contrast is reduced; its sign not predicted | −0.0202 nats before partialling, +0.0037 after (per subject −0.0202 to +0.0007): "met" | met | Results 3; S3 Text §4; Fig 4 |
| B16b, the whitened series' spectrum (21 Sep 2026, 14:03 UTC) | The AR(1) residual keeps most of its power in band (share above 0.9) with r₁ near 0.75; the AR(p ≤ 5) residual's in-band share falls to about 0.5–0.6; at p = 10 and 20 the share falls further towards 0.28 as r₁ approaches zero | AR(1) residual 0.996 in band, r₁ 0.76; AR(p) share 0.655 (against 0.5–0.6: "missed in one constant"); p = 10 and 20 shares 0.479 and 0.391 with r₁ 0.09 and 0.00: "met in its two decisive parts" | partly met | Results 7; S3 Text §8; S11 Table |
| B17b, the calibration on the band-passed generator, the regime and (i)'s sts DiD (21 Sep 2026, 14:03 UTC) | The W = 60 sts level near the null's 1.18 rather than B17's 0.715; under (i) the sts DiD between −0.07 and −0.10 | Level 1.1883; sts DiD −0.0940 ± 0.0028: "met, and better than predicted" | met | Results 2 and 4; S3 Text §7; S10 and S13 Tables |
| B17b (i), the pure-autocorrelation residual | The residual DiD near the data's own null, +0.004 to +0.008 | +0.0027 ± 0.0014: "missed low"; recorded as a miss | missed | Results 4; Table 3; S3 Text §7; S10 Table; Methods, Pre-registration and deviations |
| B17b (ii), a coupling change | Residual negative for either sign of Δc | Residual +0.0003, −0.0002, −0.0004, −0.0002, each inside its replicate SD: no response, because the construction injected a thirtieth of the AR(1) generator's lagged structure at the same c; "uninformative as implemented"; the rule "where B17 and B17b differ, quote B17b" departed from for these rows, as recorded | missed (not met; the construction's, as the record explains) | S3 Text §7; S10 Table; Methods, Pre-registration and deviations |
| B17b (iii) and (iv) | (iii) additive; (iv) level lowered, residual near its (i) value | (iii) +0.0027 against +0.0027 − 0.0002; (iv) level 1.1668, residual +0.0031: "met" | met | S3 Text §7; S10 Table |
| B17b, the population reference for B17 | Under (i) a change of about −0.08 (1.19 → 1.11); for (iv) − (i) a level difference of about −0.045 | −0.0816 (1.1936 → 1.1121) and −0.0463: "both as predicted" | met | S3 Text §7; S13 Table |
| B21 (a), the inverted intervals (23 Sep 2026, 11:31 UTC) | Wider than the committed percentile intervals by 5–20 %, and excluding zero exactly when the exact p is below 0.05 | Width ratios 1.072–1.191 for 929 of 932 quantities ("met for those"); the three outside are S9 Table cells whose committed limits were printed at five decimals; zero excluded exactly when p ≤ 0.05 for 932 of 932 | met (three ratios set by the committed rounding) | Methods, Inference; S17 Table |
| B21 (d), the Fisher-z intervals and the disattenuated ratio | The Fisher-z intervals of the known correlations as stated; the disattenuated ratio's interval wide, with its lower limit below 0.7 | Fisher-z intervals met to rounding; the ratio +0.951 [+0.617, +0.991] on ts_gsr (met) and +0.987 [+0.901, +1.069] on ts_demean ("the prediction is missed") | partly met | Results 2; S3 Text §5 |
| B22 (a), A_other (23 Sep 2026, 11:31 UTC) | ts_gsr: level +0.001 to +0.004, DiD −0.003 to +0.001; ts_demean: level −0.001 to −0.004, DiD −0.005 to 0; A_same near A_other | ts_gsr +0.00196 and −0.00059; ts_demean −0.00251 and −0.00249; A_same beside: "met" | met | Results 4; Table 3; S3 Text §6 |
| B22 (b), the slope B | Level about +0.020 (ts_gsr) and +0.024 (ts_demean); the DiD negative if a shared slow structure weakened, near B24's expectation if not | Levels +0.02019 and +0.02441; the ts_gsr DiD −0.00233 lies at B24's expectation −0.00263, which is itself negative: "the levels met; the conditional prediction does not discriminate" | partly met | S3 Text §6 |
| B22 (c), D and Sym | D DiD −0.002 to −0.006 on ts_gsr; the directed part lowers the residual DiD; Sym's DiD exceeds the residual DiD | D +0.00111 [−0.00332, +0.00518], positive; Sym +0.01034 below the residual's +0.01153: "the D DiD missed; the other two missed on ts_gsr and met on ts_demean by point estimate only" | missed | S3 Text §6 |
| B22 (d), the per-SD exchange rate | Within-window SDs r₁ 0.025–0.035 and \|q\| 0.18–0.22, ratio 4–6; group level 0.010–0.016, 0.12–0.18, ratio 2–4; unstandardised slopes +5 to +6 per unit r₁, −0.40 to −0.55 per unit \|q\|, −3 to −5 per unit asymmetry | Within windows 0.0284, 0.1957, ratio 4.66 (met); group level 0.0082 and 0.0799, ratio 3.28 (the SDs below their ranges, the ratio inside); slopes +5.126 (met on ts_gsr; +4.523 on ts_demean, missed), −0.528 (met), −2.707 (missed) | partly met | Results 1; S3 Text §4 |
| B22 (e), the network spin test | Spin p below 0.05 for the unpartialled F; 0.01–0.3 for the residual map's F; above 0.05 for both contrasts | Unpartialled F p = 0.0564 ("missed, just above 0.05"); residual map's F p = 0.0009 ("below its predicted range"); contrasts p = 0.22 and 0.31 ("met") | partly met | Results 3; S3 Text §4; Fig 4 |
| B23 (a1), the population residual under a cross-lag departure at fixed (a, q) (23 Sep 2026, 11:31 UTC) | +0.0036 and +0.0037 at ±0.01, +0.0154 and +0.0158 at ±0.02 (values known from the verification of 22 Sep on another pool) | +0.00393 and +0.00329, +0.01609 and +0.01482: positive for either sign, within 11.1 % of the stated values, the order of the two signs reversed | met (sizes within 11.1 %; the order reversed) | S18 Table; Table 3 |
| B23 (a2), the aligned departure | −0.0224 at +0.01·sign(q) and +0.0297 at −0.01·sign(q) | −0.02245 and +0.02968: "met" | met | S18 Table; Table 3 |
| B23 (a3), coupling at fixed (r₁, q) | +0.0029 at +0.01, +0.0102 at +0.02 and +0.0070 at −0.02, the aligned version moving at first order | +0.00237, +0.00912, +0.00813; the aligned version first order: "partly met" | partly met | S18 Table; Table 3 |
| B23 (a4), coupling with the innovations held | Negative for either sign, about −22 δ_sym² | Negative for either sign ("met"); residual / δ_sym² −24 to −30 against about −22 ("missed") | partly met | S18 Table; Table 3 |
| B23 (a5), the shared slow component | The residual rises as the shared component weakens, about −1.2 per unit Δr₁ | Rises ("met"); per unit Δr₁ −1.72 to −1.70 and −2.89 against about −1.2 ("missed") | partly met | S18 Table; Table 3 |
| B23 (b), the W = 60 simulation | (a4) reproduces the AR(1) calibration's rows within 2 SE; (a1) at ±0.02 +0.0066 and +0.0064; (a2) −0.0102 and +0.0135; (i) +0.003 to +0.007 | (i) +0.00537 ("met"); (a4) −0.00479 and −0.00546, within 2 SE ("met"); (a1) +0.00978 and +0.01016 ("missed, about half as large again"); (a2) −0.01354 and +0.01850 ("missed, about a third larger") | partly met | S18 Table; Table 3; Results 4 |
| B23 (c), CCS-sts on the family | CCS-sts falls as r₁ rises at fixed q; the slope at (0.85, 0.25) between −2 and −0.1 | Slope −0.0134 per unit r₁, flatter than predicted, and not monotone (it falls from 0.80 to 0.87 and rises at 0.90): "missed" | missed | Results 6; S3 Text §5; S18 Table |
| B23 (d), exposure per unit of spectral difference | Δsts within ±15 % across TR 0.72–2 s while Δr₁ varies about sevenfold (+0.190 at 0.72 s and +0.165 at 2 s at β = 100, and the stated values at 200 and 400) | +0.1904, +0.1696, +0.1652 (largest 15.3 % above the smallest), the stated values reproduced: "met to rounding" | met | Discussion; S3 Text §4; S18 Table |
| B23 (e), the population change of the band-passed generator | Population Δr₁ −0.010 to −0.020 and Δsts −0.08 to −0.15 | Δr₁ −0.01629, Δsts −0.11263: "met" | met | S13 and S18 Tables |
| B23 (f), the unequal-coefficient grid | The grid extremes zero to rounding; the sign change of the excess at about 0.008 (q = 0.25), 0.035 (q = 0.5) and 0.09 (q = 0.7); none within 0.10 at (0.80, 0.70) | Extremes within 6.7 × 10⁻¹⁶; sign changes at 0.0080, 0.0345 and 0.0915; none at (0.80, 0.70): "met" | met | Results 1; S3 Text §3; S18 Table |
| B24, the check against B17b (23 Sep 2026, 11:31 UTC) | The check passes (condition (i) reproduced within one of B17b's SDs) | sts DiD −0.09441 ± 0.00222 against −0.09400 ± 0.00280, residual +0.00279 against +0.00270, each within one SD: "passed" | met | Table 3; S18 Table |
| B24, the pure-autocorrelation expectations | A_other DiD within ±0.0005, A_same within ±0.001, B within ±0.003; D −0.002 to −0.006; Sym equal to the residual DiD minus D's within 0.001 | A_other +0.00001, A_same +0.00004, B −0.00263 (met); D −0.00100, above the predicted range ("missed"); Sym −0.00006 from the identity (met) | partly met (D missed) | Results 4; Table 3; S3 Text §6; S18 Table |

Of 52 recorded predictions, 25 were met, 13 partly met, 14 missed and 0 could not be evaluated. Six further entries recorded a reading rule or an outcome mapping without predicting which branch would obtain (B2, B6, B7, the two rules of 16 September 2026, 10:23 UTC, the cross-lag budget) or recorded no prediction (B18); the branch that obtained is given in each row.

### Part B. Post hoc computations the paper quotes
...
| The coupled family (B8, 15 September 2026, entered with no prediction) | Results 1; Fig 1c; S3 Text §3; S18 Table | Analytic; entered in the pre-run entry of 15 September 2026, 07:30 UTC with no prediction |
| The leave-two-out on the sts / autocorrelation collinearity (15 September 2026, 11:09 UTC, entered with no prediction) | S3 Text §5 | 91 refits; r = 0.846 to 0.973 |
| The checks of the review of 17 September 2026 (`notes/fresh_review_2026-09-17/checks/`) | S3 Text §6 (the run-level residual against the null, p = 0.0001) and §9 (the variance ratio partialled out of the group-mean relation, r = +0.95) | Computations of the sixth review, with no pre-recorded rule |
| The leave-one-out on the primary DiD (`scripts/13_loo_did.py`, 14 September 2026) | Results 2; S4 Text | A post hoc check; the largest sign-flip p over the 28 refits is 0.0076 |
| The proportionality check (`scripts/14_proportionality.py`, 14 September 2026; its interpretation rule recorded in the script before the run) | Results 2 (S8 Table cited); S3 Text §9; S8 Table | Post hoc, specified after the primary result; reported regardless of outcome, without the rule's labels since 25 September 2026 |
| The residual's source (`notes/partB4_residual_source.py`, 14 September 2026, written after the diagnostic's numbers were seen) | Results 2 and 4; Table 3 (the pair r₁ and \|q\| DiDs); S3 Text §6; S13 Table | The cross-lag and lag-0 substitutions applied separately; the pair r₁ and \|q\| DiDs with their sign counts |
| The numbers derived on 24 September 2026 (`notes/review_2026-09-24/checks/derived_r17.out`) | Results 2 (the Fieller interval of the sts rate; the leave-one-out cross-half values), Results 3 (the cortex-only correlation; the t intervals of the partialled contrast), Results 4 (the Fieller interval of the residual rate), Results 7 (the band-limit bound), Methods (the null's own pair-r₁ DiD); S3 Text §4–5 | Computed in verifying the review of 24 September 2026, from committed per-subject DiDs and CSVs, seed 20261120; no pre-run entry |
...
Source: `notes/partB5_literature_v2.md` (revised from the full texts on 20 September 2026; `notes/partB5_literature.md` is the record of what could be read on 14 September); the scope was fixed in `notes/partB_prespec_2026-09-14.md` (B5) before the search. Tables A and B of that file are transcribed in full; the only changes are that its computation labels and review references are replaced by the section of S3 Text or S2 Text that holds the same content. Page numbers are PDF page indices as the citation audit of 20 September 2026 gives them; "not stated" means the paper does not state it.
