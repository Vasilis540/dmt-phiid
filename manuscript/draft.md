...
Table 1 gives the primary step contrast. On `ts_gsr` at W = 60, whole-brain sts fell after injection on the DMT run and rose on the placebo run, giving a DiD of −0.0809 nats [−0.1261, −0.0377], sign-flip p = 0.0038, phase-randomised p = 0.0020, negative in 13 of 14 subjects. The FD DiD in the same form was not significant, and the FD-residualised DiD kept its sign with a CI excluding zero, so the effect survives motion control under the pre-specified definition. Without global signal regression the contrast replicated (−0.1031 [−0.1558, −0.0522], p = 0.0026), and the sensitivity windows 5–14 give the same result on both variants (Supplementary Table S1). The pre-specified direction was positive; under the directional-failure rule this is a refutation of the up-regulation hypothesis. A post-hoc leave-one-out check, not pre-specified, refit the primary DiD with each subject dropped in turn: the mean ranged from −0.0944 to −0.0656 on ts_gsr and from −0.1182 to −0.0871 on ts_demean, all 28 refits kept a bootstrap CI excluding zero; the most influential single subject is subject 14, the only positive DiD on both variants, whose removal strengthens the effect (−0.0944 on ts_gsr, −0.1182 on ts_demean), while removing the largest-magnitude subject, subject 8 (per-subject DiD −0.280 on ts_gsr, −0.311 on ts_demean), weakens it but leaves it significant: −0.0656 [−0.1013, −0.0271], p = 0.0076 on ts_gsr and −0.0871 [−0.1310, −0.0408], p = 0.0051 on ts_demean (`results/loo_did_win60.csv`).
...
A post-hoc analysis, specified after the primary result with its interpretation rule recorded before it was run, asked whether synergy fell selectively or in proportion to TDMI, by applying the same DiD to the ratio sts / TDMI (Supplementary Table S8). Under the primary estimator the ratio DiD was +0.0005 [−0.0081, +0.0083], p = 0.91 on `ts_gsr` and −0.0095 [−0.0235, +0.0049], p = 0.22 on `ts_demean`; under the global fit it was +0.0204 [+0.0048, +0.0340], p = 0.025 on `ts_gsr` and +0.0071 [−0.0167, +0.0292], p = 0.56 on `ts_demean`. In the primary cell sts was 0.782 of TDMI at pre-injection baseline on the DMT run and carried 0.780 [0.653, 0.936] of the TDMI drop. Under the pre-recorded rule (a CI including zero means proportional; significantly negative, more than proportional; positive, less) three cells are proportional and one, the global fit on `ts_gsr`, is significantly less than proportional, a result that does not replicate on `ts_demean`. No cell shows a more-than-proportional synergy reduction. The windowed and global estimators give different baseline shares (0.78 and 0.90) because the per-window finite-sample bias falls on sts and on the self-transfer atoms, so the baseline share and the share of the drop are comparable within a cell and not across estimators.
...
### The decrease is proportional, not selective

Under the primary estimator the synergy decrease is proportional to the total reduction in predictable information: sts was 78 % of TDMI before injection and carried 78 % of the TDMI drop, and the ratio sts / TDMI did not change (post-hoc, Supplementary Table S8). These data therefore give no evidence that synergy is selectively affected by DMT. That fixes what the decomposition does and does not buy here. It buys the directional test: the workspace account predicted that the synergistic atom specifically would rise with intensified conscious content, the atom was measured, and it fell, so the prediction fails on its own terms. It does not buy a synergy-specific finding. A whole-brain loss of predictability that took every atom down in proportion would produce the same sts result, and the only cell in which the ratio moved, the global fit on `ts_gsr`, moved in the direction of synergy falling *less* than the rest and did not replicate. The honest summary is that DMT reduces the predictable information between regions across the board, and synergy, being most of that information, falls with it.
...
Liardi, A., Rosas, F. E., Carhart-Harris, R. L., Blackburne, G., Bor, D., & Mediano, P. A. M. (2024). Null models for comparing information decomposition across complex systems. *arXiv*, 2410.11583. Preprint, not peer reviewed.
