...
Table 1 gives every atom's whole-brain mean in the DMT pre-injection windows and its primary DiD under both redundancy functions (`ts_gsr`, W = 60; `notes/review_results/partB/ccs_tables.md`). Under MMI the table has the structure the closed form predicts: the self-prediction atoms xtx and yty are +0.6273 and +0.6125 nats and their sum +1.2398 is within 8 % of sts (+1.1554); rts and str are +0.5669 and +0.5667; and four atoms are large and negative (xts −0.5352, yts −0.5358, stx −0.5350, sty −0.5357; negative atoms are permitted by the lattice's Möbius inversion, Mediano et al., 2021), so that the "synergy block" sts + rts + str = +2.2890 is offset by −2.1417 within a TDMI of +1.4772. Under DMT the four negative atoms rose (+0.0399 to +0.0404) while sts, xtx, yty, rts and str fell (−0.0809, −0.0523, −0.0396, −0.0407, −0.0408); the block's net change is −0.0017 of the −0.1037 change in TDMI. Under CCS the same TDMI is distributed differently: xtx and yty and their DiDs are almost unchanged, rtr roughly doubles (+0.0916 against +0.0388), rts, str and the four negative atoms are ≈ 0 to +0.06, and sts is −0.0387, 3 % of the MMI value in magnitude and negative. sts ≈ xtx + yty holds under MMI only; the same pattern holds at the global fit and on `ts_demean` (CCS-sts −0.0335, −0.0255, −0.0297 against MMI +1.3085, +1.1004, +1.2403).
...
Table 3 summarises the diagnostic (`notes/review_results/partB/diag_tables.md`, `notes/review_results/inference_rows_diag.csv`). The prediction from each pair's measured (a_x, a_y, q) alone reproduces the observed sts level to within 4.3 % (observed 1.1377, predicted 1.1865; `ts_gsr`, W = 60), over-predicting; the residual's SD across the 392 subject × run × window cells is 0.0161 against 0.0790 for the observed sts, per pair within a window the squared correlation between predicted and observed sts is 0.796, and at the run level the residual is −1.1 % (`ts_gsr`) and +0.1 % (`ts_demean`). The symmetric prediction with a_x = a_y (the map's family) over-predicts by 16 % (1.3212). The predicted DiD has the observed sign in every cell and exceeds the observed DiD in magnitude by 14 % (−0.0924 against −0.0809; `ts_gsr` W = 60), 27 % (W = 30), 18 % and 29 % (`ts_demean`); per subject predicted and observed DiDs correlate at 0.989, 0.987, 0.990 and 0.981.

The residual DiD (exploratory throughout this paragraph) is +0.0115 [+0.0021, +0.0211], sign-flip p = 0.042, phase p = 0.019, negative in 4 of 14, FD-residualised +0.0100 [+0.0008, +0.0185] (p = 0.058), and concentrated in the early set (+0.0179, p = 0.0012; late +0.0064, p = 0.32). Against the recorded prediction, neither outcome obtained as worded (exploratory): the residual DiD is not near zero by this study's conventions, since its CI excludes zero and both nulls are below 0.05 in all four windowed cells, and it is not uncorrelated with r₁, since per subject it correlates with the autocorrelation contrast at −0.780, −0.825, −0.597 and −0.604, the subjects whose r₁ fell more showing the larger over-prediction. The pre-specified signature of an autocorrelation-independent component (significant and uncorrelated with r₁) is not met, and the residual is reported as exploratory and unresolved. A post-hoc supplement (`ts_gsr` W = 60 only; `notes/review_results/partB/residual_source.log`) locates the whole residual in the cross-lag substitution: replacing only the two cross-lag entries gives 1.1866 (the lag-0 substitution alone 1.1372); the measured cross-lag correlations deviate from a_y q with mean +0.00014 and SD 0.1128 across pairs within a window; the residual is more negative where pair a and |q| are higher (r = −0.736 and −0.711 across cells), and the positive residual DiD is that over-prediction shrinking as a and |q| fall under DMT (exploratory). The diagnostic's only model assumption is corr(x_t, y_{t+1}) = a_y q, and on the post-hoc supplement's reading the residual is the cost of that assumption on these data (exploratory).
