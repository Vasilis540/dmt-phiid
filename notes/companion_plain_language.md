...
**(a) What it says.** The analysis code, verification scripts, the six adversarial reviews (the fourth of them the verification of the correction note), the Part B computations and the manuscript drafts were produced with the assistance of an AI system (Claude, Anthropic) working under V.S.'s direction; session outputs are committed with the model and the session of each contribution identified in the commit trailers (several models were used across the sessions, which is why no single model is named); every number was checked against its source file before commit, the first three reviews re-traced every number of the draft they read and the fourth every number the revision had introduced, with findings (including errors found) in `notes/`, the fifth review's one finding is quoted in the record, and the sixth re-traced about 200 numbers of the draft it read, with its checks in `notes/fresh_review_2026-09-17/`; V.S. takes responsibility for analysis, text and claims. Journal-specific wording [TK].
