...
**(c) What a sceptic asks.** *If r₁ explains 114 %, why is there a "remainder" at all?* Because 114 % is an over-prediction, and the over-prediction (the residual DiD, +0.0115) is itself significant at p = 0.042 and only partly reproduced by the null; Results 4 ends its account of the residual with what "is not located": what produces the run-level cross-lag deviation, and the rest of the run-level residual and of the residual DiD. *Why recommend CCS if you also say CCS has no established reading?* The paper is careful: "consider CCS", and Discussion, "Redundancy functions", says "CCS is not offered as the corrected estimator, only as the one whose sts does not carry the autocorrelation contrast the way MMI-sts does".
...
**(c) What a sceptic asks.** *Your first test said pooling was ruled out; your second says it is present. Why believe the second?* Because the first could not tell the two apart on `ts_gsr` — the q > 0 and q < 0 pairs cancel in it — whereas the second is the quantity the mechanism predicts, its prediction was recorded before it ran, it is positive in all 14 people, and across runs it tracks the residual at −0.767 against the plain average's −0.277 (Results 4). *So pooling explains the run-level residual?* No: the statistic shows the signature, at a size that on the family's scale is somewhat under half the residual, and it does not distinguish pooling from lagged interaction that follows the sign of q or from finite sampling, whose value of the statistic was not computed (Results 4; Limitations). *Why is `ts_demean` negative?* The paper does not interpret it; there the deviation is negative for pairs of both signs of q, and that version's run-level residual is +0.1 %.
