...
*Round 13 (20 September 2026; record, "Text corrections of round 13"): the paper's sentences listed in the citation audit's F1–F19 and the plan's Q1–Q8 (`notes/review_2026-09-20/`) changed. The units below that restate them are corrected in this round only where the restatement is now false about a source or a number — the residual–CCS correlations (Results 4 unit), Luppi et al. (2022)'s stated TR, band, replication figure and surrogate test (Discussion — A distinction for applicability unit), and the sources described as not read or blocked (all nine studies, Mediano et al., 2025 and Raut et al., 2020 were read in full on 20 September) — and are otherwise left as written on 15–18 September; the companion is rewritten after the restructuring round (round 14).*
...
**Use of AI tools** — The declaration (a subsection of Materials and methods since 18 September 2026; before that the back-matter "AI-use statement") of which parts of the work were produced with an AI system's help, how its outputs were checked and who takes responsibility for them.
...
**COBIDAS** — A reporting checklist for MRI studies from the Organization for Human Brain Mapping, filled in as S4 Text.
...
**cross-lag deviation** — For a pair, the measured cross-lag correlation minus what the AR(1) model predicts for it (a_y q or a_x q); at the run level the paper reports its signed mean over pairs and its sign(q)-weighted mean, and the second is the test of the pooling mechanism.
...
**Lempel–Ziv complexity** — A measure of how compressible a signal is, used with EEG as an index of signal diversity in the original analysis (S1 Text).
...
**shared slow component** — A slowly varying signal present in both regions of a pair on top of each region's own faster fluctuations; it makes the cross-lag correlation exceed what the AR(1) model predicts by λ(1 − λ)(a_s − a_n), with λ its share of each region's variance and a_s, a_n the lag-1 autocorrelations of the shared and regional parts — one of the stationary look-alikes of pooling in Results 4.
...
**sign(q)-weighted mean** — The average over pairs of a pair quantity multiplied by the sign (+1 or −1) of the pair's correlation q, so that effects whose direction follows q add up instead of cancelling between positively and negatively correlated pairs; for the cross-lag deviation, the statistic the pooling mechanism implies.
...
**tier (tier-2)** — The original analysis's label for its second-level claim, that synergy tracks the intensity ratings, which was declared void under its own controls (S1 Text, S2 Table).
...
**(c) What a sceptic asks.** *"Dominates" — dominates what, measured how?* The answer is in Results 2 (the derivative ratio at the operating point, 32; minimum on the |q| ≤ 0.6 grid 6.55) and Discussion, "What the finding is and is not" ("on this dataset the per-pair diagnostic's prediction from each pair's (a_x, a_y, q) gives the sign and 114 % of the magnitude of the sts change (Results 4), and the per-subject collinearity is 0.95 (which includes estimation error the two DiDs share; Results 3)"). *Isn't "analytic account" too strong for a formula that the data violate?* The paper concedes this itself: Results 1 says the closed form "is an account of the estimator's structure, exact on the family, and only approximate on these data", and the Limitations repeat it.

---

### Abstract — Background

*(Since 18 September 2026 the Abstract has no section headings — PLOS abstracts are unstructured — and the four units below follow its Background–Methods–Results–Conclusions flow as sentences.)*
...
*(Revised 16 September 2026: the Abstract was cut to 300 words. It keeps the identity and the 32-fold ratio, the regional r = 0.863 with the cortical Spearman and its spin p, the DMT DiD, the r₁ DiD and their r = 0.953, the diagnostic's 114 % and the residual with the null's share, CCS-sts not tracking r₁ and rising (exploratory), and the recommendation. The "holds to 7 %", the −0.084 / +0.039 excess, r² = 0.745, the 4.3 % level, the 0.989 per-subject tracking, "by 14 %" and the "(N = 14 …)" parenthesis described below left the Abstract (the within-window CCS correlations, r ≈ −0.01 against +0.7, and the spatial-map implication were restored the same day); each stays in Results 1–4, and the "≈20 effective samples" of the Methods paragraph is in Methods, Estimator. Revised again on 17 September, after the fresh adversarial review: the 32-fold ratio is now stated at BOLD pairs' operating point, the null's share of the residual DiD gains "every null value inside its CI", the CCS bracket reads "across pairs within a window, |r| < 0.02 against +0.7", and the Conclusions add "and lagged interaction does not change comparably". Revised again on 18 September for PLOS Computational Biology: the Abstract is unstructured (one paragraph, the four bold headings dropped, the flow kept as sentences) and at most 300 words; in the commissioned order the split-half sentence ("Its DiD correlates with the residual's at 0.80; a split-half test could not tell shared signal from window noise; no autocorrelation-independent component is reported") and the clause "so spatial synergy maps are exposed to regional differences in r₁" left it (296 words); the CCS bracket's numbers stay; both dropped statements stay in Results 4 and Results 2.)*

**(a) What it says.** The formula came out as sts = −ln(1 − r₁²) + ½ ln(1 − r₁²q²), and this equals the two self-prediction atoms plus the double-redundancy atom: sts = xtx + yty + rtr. In words: on this model, "synergy" is each region predicting itself, plus a little redundancy, and four other atoms come out *negative* to balance the books (more on negative atoms under Results 1). The slope of sts with respect to r₁ is 32 times steeper than its slope with respect to q at the operating point where real pairs sit. On the data the identity holds to 7 %, but the small leftover has the wrong sign: sts is 0.084 nats *below* xtx + yty, whereas the model says it should be *above* by rtr = 0.039. (A *nat* is the unit of information when you use natural logarithms; one nat is about 1.44 bits.) Across the 115 regions, on the placebo run before injection, each region's synergy follows its own r₁, as predicted before the test was run: Pearson r = 0.863, so r² = 0.745 — three-quarters of the between-region differences in synergy go with differences in r₁ — and on the cortical parcels Spearman 0.771 with spin p < 0.0001; so a brain *map* of synergy, not only a before/after contrast, is exposed to autocorrelation on these data. Under DMT the whole-brain sts fell (DiD −0.0809 nats, p = 0.0038), and so did r₁ (−0.0146, p = 0.0106, a measurement that was not planned in advance), and across the 14 people the two changes track each other at r = 0.953. The per-pair prediction reproduced the sts level to 4.3 % and the per-person DiD at r = 0.989, but over-predicted the group DiD by 14 %; a simulation with only finite-sample error in it (a computation of the second adversarial review, not pre-planned) accounts for a third to two-thirds of that leftover (+0.0037 to +0.0077 of +0.0115, p = 0.042), and every one of those null values lies inside the leftover's confidence interval. Under CCS, sts does not follow r₁ across pairs (|r| < 0.02 across pairs within a window, against +0.7 for MMI) and it went *up* under DMT (labelled exploratory); its per-person change correlates with the leftover's at 0.80, but a test designed to tell real shared signal from shared noise, its rule recorded before the run, had been set with a threshold no reliable measurement could reach, so it could not decide; the paper reports no autocorrelation-independent effect.

**(b) Why it is there.** It is the compressed evidence chain: formula → map → data agreement → data disagreement → regional map → DMT contrast → residual → null → CCS → undetermined. The clauses come from Results 1 to 4; the regional map is Results 2.

**(c) What a sceptic asks.** *You say the identity "holds to 7 %" and in the same breath that the sign of the excess is wrong — which is it?* Both: 7 % is the size of the gap between sts and xtx + yty relative to their size; the sign of that gap is opposite to the model's. Results 1 gives the numbers (1.1554 against 1.2398; −0.084 against +0.039). *"Not significantly per subject (N = 14)" — not significantly what?* The abstract compresses: it means the per-subject CCS-sts DiD is not significantly correlated with the per-subject autocorrelation DiD (Results 3: r = −0.420 at the primary cell, all four cells p ≥ 0.127). The revised abstract of 15 September said it: per subject the CCS-sts DiD is not significantly correlated with the r₁ DiD, a bound on the dependence rather than independence; the 300-word Abstract of 16 September dropped the clause, and Results 3 carries it.

---

### Abstract — Conclusions

**(a) What it says.** Whenever two states or two groups differ in r₁ and do not differ comparably in lagged interaction (one region's past carrying information about the other's future), an MMI-sts contrast between them will be mostly the r₁ difference. The per-pair diagnostic's prediction from each pair's (a_x, a_y, q), which gives the sign of the sts change and 114 % of its size, now sits in the Abstract's Results. Three practical instructions: always report r₁ next to sts; run the diagnostic; consider CCS or a longer lag.
...
**(c) What a sceptic asks.** *If r₁ explains 114 %, why is there a "remainder" at all?* Because 114 % is an over-prediction, and the over-prediction (the residual DiD, +0.0115) is itself significant against zero at p = 0.042 and not distinguishable from the null, every null value lying inside its interval; Results 4 ends its account of the residual with what "is not located": what acts inside the windows, and what produces the rest of the run-level residual (the residual DiD's remainder being not established). *Why recommend CCS if you also say CCS has no established reading?* The paper is careful: "consider CCS", and Discussion, "Redundancy functions", says "CCS is not offered as the corrected estimator, only as the one whose sts does not carry the autocorrelation contrast the way MMI-sts does".

---

### Author summary (added 18 September 2026)

*(New in the PLOS Computational Biology form of the paper: a section of 150–200 words in the first person plural, after the Abstract, non-technical and distinct from it, with at most one number.)*

**(a) What it says.** Functional brain imaging asks how regions carry information together, not only which regions are active. A widely used measure, from integrated information decomposition, is synergy: information a pair of regions holds jointly and neither holds alone. Synergy has been reported to differ across the cortex and to pick out regions whose integration falls under anaesthesia and in disorders of consciousness, and is read as a mark of integrative processing. We show that, with the estimator almost all of these studies use, the synergy value mostly reflects something simpler: how smooth each region's signal is over time (its lag-1 autocorrelation). We derive this on a solvable mathematical family and confirm it on functional MRI data from fourteen volunteers scanned under the psychedelic DMT and under placebo. Under DMT the measured synergy fell, but so did the smoothness of the signals, and the apparent fall in synergy was the fall in autocorrelation. We had set out to test the opposite hypothesis, that DMT raises synergy, and we report its refutation and what we found instead. We recommend that studies report autocorrelation beside synergy and run the diagnostic we provide, which says how much of a synergy contrast autocorrelation accounts for.

**(b) Why it is there.** PLOS Computational Biology asks for it: a summary a non-specialist can read, saying what the measure is used for, what was found, how the study came about and what readers should do. It says in words what the Abstract says in numbers — the smoothness of a region's signal over time is its lag-1 autocorrelation, and "the apparent fall in synergy was the fall in autocorrelation" is the per-subject r = 0.953 and the diagnostic's 114 % of Results 3 and 4.

**(c) What a sceptic asks.** *"Mostly reflects" — how much is mostly?* The Author summary carries no numbers by design; the Abstract and Results 2–4 give them (the 32-fold derivative ratio at the operating point, r = 0.863 across regions, 114 % of the DMT contrast). *You say you tested the opposite hypothesis first — where?* The History of the study (Materials and methods) and S1 Text.
...
**(b) Why it is there.** A reader needs to know that the design is within-subject (each person is their own control), that the ratings do not belong to the analysed run (which matters for S1 Text), that the filtering already imposes high r₁ (which is the mechanism), and that one bad parcel was removed before any result rather than after. The licence sentence and the subject-order sentence are the paper's due diligence on reusing someone else's data.

**(c) What a sceptic asks.** *If the ratings are from another session, the whole "synergy tracks intensity" design was broken from the start, wasn't it?* The paper says the ratings "enter no result in the main text" and puts the original analysis in S1 Text, where the intensity-tracking claim was void under its own controls anyway (S2 Table). *Subject order verified at one subject only?* Stated as such: the paper (Methods, Dataset) cites the record entry "Subject alignment across files" and does not say that the data authors have confirmed the order. If the order were wrong, the motion control would regress each subject's values on someone else's motion.
...
**(c) What a sceptic asks.** *A web search on one day is not a systematic review.* The paper does not claim it is; it calls the TR range "a scoping rule, not a judgement" and (since 20 September) records that every study was read in full from its PDF. *Why exclude the most important study?* Discussion, "A distinction for applicability", addresses Luppi et al. (2022) separately and explicitly.
...
**(c) What a sceptic asks.** *Everything left over points in the direction of your original hypothesis: the residual DiD is positive, largest in the drug-present windows, and shares 80 % of its per-person variance with a CCS-sts increase. Aren't you burying a real DMT effect under "exploratory"?* The paper's answer is procedural and evidential: procedurally, the residual arose after four specification changes and the closure entry forbids a DMT-specific claim from any of them; evidentially, a stationary null with reasonable free choices produces a third to two-thirds of it, the run-level residual is absent on `ts_demean`, and the reliabilities (0.49, 0.30) are too low to say whether the shared component is signal. What it cannot say is that there is no effect — what produces the rest "is not located". *"A mean coupling of about +0.006" — would do what, and is it there?* It would produce a residual the size of the run-level one (about −0.01 nats; −0.0137 on `ts_gsr`), and it would show as a sign(q)-weighted cross-lag deviation of about +0.006; at the run level the data have +0.0034, of which finite sampling is a minor part (Results 4).

#### The run-level cross-lag deviation and its budget (added to Results 4 after the companion; its reading revised after the fifth review and twice on 16 September)

**(a) What it says.** Results 4 names one candidate for the run-level residual (the −1.1 % on `ts_gsr`): *pooling* — fitting one Gaussian to a run whose segments have different (a, q), so that the run's cross-lag correlation need not equal the product of its a and q. For a pair whose correlation q is positive, pooling pushes the measured cross-lag correlation *above* a_y q; for a pair whose q is negative, *below*; and at the run level 54.5 % of the `ts_gsr` pairs have q < 0. The test measures, per person and run, each pair's two cross-lag correlations minus what the AR(1) model predicts for them (a_y q and a_x q), from the same run-level matrices as the run-level rows of Table 4.

It was first computed as a plain (signed) average over pairs, with a prediction written into the record beforehand — although the `ts_gsr` average had already been glimpsed once, while the run-level numbers of Table 4 were being reproduced, which the record's pre-run entry and now the paper disclose. The prediction, as commissioned: negative on `ts_gsr` and near zero on `ts_demean` if pooling explains the residual, near zero on both if it does not ("near zero" meaning a subject-bootstrap CI that includes zero); the record and the paper note that the commissioned sign is reversed relative to the coupled family's, on which a positive deviation is what lowers sts below the prediction where q > 0. The outcome fell outside both branches: +0.00009 [+0.00005, +0.00013] on `ts_gsr` (sign-flip p = 0.0004, positive in 13 of 14) against a pair-level SD of 0.0390; −0.00738 [−0.00902, −0.00588] on `ts_demean` (p = 0.0001, negative in 14 of 14); across the 28 runs it correlates with the run-level residual at −0.277 and −0.890. The previous version of the paper read this as "pooling does not account for the run-level residual".

A fifth review showed why that reading was not licensed. Flipping the sign of one region's series flips both q and the deviation and leaves every atom unchanged, so the deviation that lowers sts is positive for the q > 0 pairs and negative for the q < 0 pairs, and in a plain average the two kinds of pair cancel whether or not pooling operates — as they do: +0.00384 on average among the q > 0 pairs, −0.00304 among the q < 0 pairs. The +0.006 the mechanism needs had been worked out at q = +0.25. The right average multiplies each pair's deviation by the sign of its q, the *sign(q)-weighted mean*. Its prediction, recorded before it was run: positive and of order +0.006 on `ts_gsr` if pooling operates as described (read as within a factor of two, with a CI excluding zero); near zero or far below that if pooling does not account for the residual. It gives +0.00340 [+0.00300, +0.00380] on `ts_gsr`, sign-flip p = 0.0001, positive in 14 of 14 people (DMT run +0.00381, placebo run +0.00299): a little over half of +0.006, inside the band, near its lower edge. Across the 28 runs it tracks the run-level residual at −0.767, where the plain average managed −0.277. The slope of the deviation on q across pairs is +0.0163 [+0.0141, +0.0187], positive in 14 of 14. The +0.006 was the requirement for a residual of about −0.01 nats, and the observed one is −0.0137; on the family +0.0034 would give 44 % of it at the operating point, or 37 % at the pairs' run-level average a and |q| (0.8666, 0.1945) — a scale, the paper says, not a decomposition, because the scatter of the deviations about their mean also moves sts. On `ts_demean`, where no prediction was made, the sign(q)-weighted mean is −0.00262 [−0.00495, −0.00037] (sign-flip p = 0.0532, positive in 5 of 14) and the slope +0.0211, positive in 14 of 14; there the deviation is negative on average for both kinds of pair (−0.00582 where q > 0, −0.01441 where q < 0), and the paper reports this without interpretation. What that establishes: at the run level the `ts_gsr` cross-lag correlations depart from the AR(1) model in the direction of each pair's q, in every person, at a size that on the family's scale corresponds to 44 % of the run-level residual (37 % at the pairs' run-level mean point, a 0.8666, |q| 0.1945).

It does not say what produces the departure, and pooling has three look-alikes that leave the same signature: lagged interaction whose sign follows the pair's correlation; finite sampling; and a *shared slow component* — both regions carrying a common signal s that changes more slowly than their own regional fluctuations (lag-1 autocorrelation a_s above a_n). In the general form of Methods, with λ_x and λ_y the shares of s in the two regions' variance, q = ±√(λ_x λ_y) and the deviation is q(1 − λ_y)(a_s − a_n): positive when the shared part is slower, following the sign of q, and present in a perfectly stationary process. And the last two look-alikes are not two: any stationary pair's lag-0/lag-1 structure is that of a VAR(1) with A = Γ₁Γ₀⁻¹ (Methods), so the worked example λ = 0.25, a_s = 0.95, a_n = 0.80 is exactly the coupled family at coefficient 0.83, c = 0.03 and q_ε = 0.093, and no τ = 1 quantity in the paper — the sixteen atoms, the residual, CCS-sts — separates common slow drive from lagged coupling.

On 16 September two computations first tried to separate the accounts by window length and by a null (the W = 60 value with the sign of each window's own q, +0.00611, and the null's homogeneous-filter value at run length, +0.00025), and were then withdrawn by a correction note: the window's sign is estimated from the same 60 samples as the deviation and selects on them (the null's own value with that weight is +0.00348, about half of the +0.00611), and the null had been read at a different density of q̂ near zero than the data's (its solved value is +0.00029 to +0.00046, not +0.00025). Their readings are superseded and the values kept in S9 Table. What replaced them is an *exact budget*. With the sign of each pair's run-level q used throughout, the run-level statistic splits, to machine precision, into a within-window term (the deviation inside each 2-minute window, variance-weighted), a pooling term (windows differing in a, q or variance under one fit — the mechanism Results 4 describes), a window-means term (slow variation beyond two minutes, shared or not) and a small boundary remainder: δ_run = δ_within + δ_pool + δ_means + ε. The same budget was scored on the finite-sample null, now solved so that its run-level mean a, mean |q̂| and fraction of pairs with |q̂| < 0.05 equal the data's for each run type, in six configurations, and on two controls, a shared slow component (λ = 0.2) and pooling (q and filter changing at TR 240); the rules for reading it were written down before the run. Result, `ts_gsr`: the null reproduces +0.00029 to +0.00046 of the +0.00340 — finite sampling *a minor part* (8–14 %, the paper's rounding of F = 0.085–0.135; the null's value depends on the density of q̂ near zero, to which each configuration is solved); of the null-corrected +0.00301 [+0.00261, +0.00340], the within-window term is *most* (+0.00250, 83–84 % in every configuration, where the shared-slow-component control puts 90 % of its signature and the pooling control 30 %, where the within-window term equals the null's, +0.00040, i.e. it is finite sampling), pooling *a minor part* (+0.00046, 15 %) and the window means *a minor part* (+0.00004); and pooling is larger on the DMT run (DMT − placebo +0.00039 per person, sign-flip p = 0.0006, positive in 12 of 14; 11 % of that run's signature). So the departure lives inside 2-minute windows, where pooling across windows cannot act; pooling is not its principal account, and the DMT run carries about a tenth more of it. On `ts_demean` the budget is computed and not read (δ_run −0.00262, of which the within-window term −0.00226); the paper reports the negative value and does not interpret it. What acts inside the windows — common slow drive or lagged coupling (one account at τ = 1), non-stationarity faster than two minutes, or a within-window finite-sample effect the null as specified does not reproduce — stays open, and the closure entry of 16 September says no further computation on it will be run before the draft is circulated.

**(b) Why it is there.** It is the test of the only mechanism the paper names for the run-level residual. Its history is part of why it is there: the first version measured an average that could not tell the two answers apart, and the paper drew from it a conclusion the data did not support; the statistic the mechanism implies reverses that conclusion, and the paper now reports both statistics, the order in which they were computed, and what had been seen before each prediction was written. The two computations of 16 September then tried to move the answer from pooling to a stationary mechanism by window length and by a null, and were withdrawn the same day for defects a correction note records; the exact budget, with its rules written first, is what now carries the reading: most of the departure lies inside 2-minute windows, and pooling is a minor part, larger on the DMT run.

**(c) What a sceptic asks.** *Your first test said pooling was ruled out; your second says it is present. Why believe the second?* Because the first could not tell the two apart on `ts_gsr` — the q > 0 and q < 0 pairs cancel in it — whereas the second is the quantity the mechanism predicts, its prediction was recorded before it ran, it is positive in all 14 people, and across runs it tracks the residual at −0.767 against the plain average's −0.277 (Results 4). *So pooling explains the run-level residual?* No. The budget puts 83–84 % of the null-corrected departure inside 2-minute windows, where pooling across windows cannot act, and pooling at 15 %, a minor part — though a real one, larger on the DMT run by about a tenth of that run's signature (Results 4; S9 Table). *Your W = 60 value was 1.80 times the run-level one — what happened to that?* It was an artefact of taking the sign of q from each window, which is estimated from the same 60 samples as the deviation; the null alone gives +0.00348 with that weight. With the run-level sign the within-window statistic is +0.00245, and the budget's within-window term +0.00291; the correction note of 16 September records the defect and the value is kept, marked superseded, in S9 Table. *Then what is it?* The paper says it cannot tell: common slow drive and lagged coupling give the same lag-0/lag-1 structure and therefore the same atoms, residual and CCS-sts at τ = 1, and neither is separated from faster non-stationarity or from a within-window finite-sample effect the null does not contain (Results 4; Limitations). *Why is `ts_demean` negative?* The paper does not interpret it, and says only that a shared part slower than the regional parts would not produce a negative value; there the deviation is negative for pairs of both signs of q, and that version's run-level residual is +0.1 %.
...
**(c) What a sceptic asks.** *Your "observed level" here is 1.1377 but Table 2 says the DMT pre-injection mean is 1.1554; which sts level does the 4.3 % refer to?* The revised Table 4 caption says: the level columns are means over all subjects, both runs and all windows (the 392 subject × run × window combinations at W = 60), and in the run-level rows the whole-run plug-in sts averaged over subjects and runs — not the DMT pre-injection means (1.1554 at W = 60, Tables 1 and 2; 1.3085 at the global fit, Results 1 and 6). *The residual DiD's CIs exclude zero in every cell but the sign-flip p is only 0.042 at the primary cell — which do you trust?* The paper's weighting rule settles it: p between 0.01 and 0.06 is "reported as such and is not treated as an established effect".
...
**(a) What it says.** The paper says nothing about whether the brain has synergy in the intended sense; it says what the estimator responds to. On the AR(1) family the Gaussian-MMI synergy atom is self-prediction plus rtr, balanced by four negative atoms that CCS does not have, and its dependence on r₁ dominates its dependence on q everywhere BOLD pairs sit. That is a property of the estimator on a family without lagged interaction; with interaction, sts also moves at fixed (r₁, q). The consequence is narrow: when two states differ in r₁ and not comparably in lagged interaction, the MMI-sts contrast between them is dominated by that difference, scaled by ∂sts/∂r₁, whether or not anything "synergy" is meant to capture has changed. On this dataset the cross-lag departure through which lagged interaction would show at τ = 1 is measured and differs little between the runs (+0.0038 on the DMT run against +0.0030 on placebo; about 0.0015 nats on the family's scale). On this dataset the per-pair diagnostic's prediction from each pair's (a_x, a_y, q) gives the sign and 114 % of the size of the sts change, and the per-person collinearity is 0.95 (which includes estimation error the two changes share) — and that is what the dataset shows, not "sts change is autocorrelation change": the pre-specified criterion under which it would have shown more (a residual DiD near zero) was not met. Three things the data add are stated as they are: the excess has the wrong sign (sts 0.084 below xtx + yty where the family puts it above by rtr), so the closed form is a structural account, not an exact model; the residual is positive in every cell (p = 0.042, not "established" under the weighting rule), larger in the early windows, in the direction of the original hypothesis, a third to two-thirds of it reproduced by a stationary null that does not match non-stationary runs, the rest not distinguishable from that null while the run-level residual on `ts_gsr` exceeds it, with the cross-lag departure behind the run-level residual (sign(q)-weighted deviation +0.0034 on `ts_gsr`, positive in 14 of 14; 44 % of that residual on the family's scale) placed by an exact budget mostly inside 2-minute windows (83–84 %), finite sampling and pooling a minor part each and pooling larger on the DMT run, with what acts inside the windows not identified — common slow drive and lagged coupling being one account at τ = 1 — and the negative `ts_demean` value (−0.0026) reported without interpretation; and the residual's per-person variance is largely shared with CCS-sts, which the flawed split-half rule could not classify (cross-half 0.82 of the ceiling, its disattenuated value; reliabilities 0.49, 0.30). No autocorrelation-independent synergy change is reported; the residual and CCS-sts stay exploratory; an independent dataset with a pre-specified test would settle it.

**(b) Why it is there.** It draws the boundary of the claim in both directions and puts the three uncomfortable facts (wrong-signed excess, unexplained residual, CCS-sts co-movement) in the opening section of the Discussion rather than the last (since 16 September that section is four paragraphs: the estimator and the narrow consequence; the three things the data add; the shared variance with CCS-sts and the undetermined split-half test; what is and is not claimed).
...
**(a) What it says.** Two uses of MMI-sts must be distinguished. (1) A *contrast* — between states, between groups, or across a session — is exposed wherever the compared conditions differ in r₁: BOLD autocorrelation changes with anaesthetic depth and in disorders of consciousness (Huang et al., 2018), under DMT (this dataset), and within a placebo run with time (the placebo run's r₁ drift carries 0.34 of the DiD here); between-group designs are exposed to any group difference in r₁ from physiology, motion, age or scanner. (2) A *spatial map* of synergy across regions is exposed through regional differences in r₁: higher r₁ gives higher sts with rtr almost unmoved, so a regional autocorrelation gradient would produce a synergy-minus-redundancy gradient of the same spatial form, and within a single window half the between-pair variance of sts here is pair r₁. The applicability table (S3 Text) gives every readable study the same conditional sentence; it is what the map predicts, not evidence about any study, since none reports regional r₁; every study gets the sentence and none is singled out. Luppi et al. (2022) falls outside the table's TR range (a scoping rule). By the map it would be the most exposed dataset: at its stated TR of 0.72 s and 0.008–0.09 Hz band (read from the paper on 20 September) the ideal-band-pass r₁ is ≈ 0.97, where ∂sts/∂r₁ = 32.8 against ∂rtr/∂r₁ = 0.06; its measured regional r₁ is not reported. Its primary analysis is HRF-deconvolved, the gradient is replicated without deconvolution (its Extended Data Fig. 2c) with, the authors report, negligible effects at that TR, and this paper's own deconvolution evidence is from TR 2 s data and does not transfer. Since 20 September the paper also states that study's own test of the exposure: synergy and redundancy recomputed on phase-randomised surrogates that keep each region's autocorrelation and remove the correlations between regions, which on the family is the q = 0 line where sts = 2 min(S_x, S_y) and rtr = 0, so the surrogate gradient is, up to noise, the regional r₁ gradient; its correlations with the macroscale maps were small (one of six significant) against 0.40–0.54 for the observed gradient — evidence against the exposure for those associations on that dataset, and the only such evidence in the table; the correlation of the observed map with regional r₁, which this paper measures on its own data (+0.792 for sts − rtr), is not reported there. Two limits on reach: because rtr + sts = TDMI (so ∂rtr/∂q = −∂sts/∂q) and ∂rtr/∂r₁ ≈ 0, a change in q moves synergy and redundancy oppositely by equal amounts while a change in r₁ leaves rtr almost untouched — so a pattern of synergy and redundancy moving in opposite directions (Down et al., 2026) is the q signature or a mixture, not the r₁ signature (a family statement the data follow only loosely: MMI rtr fell by a tenth of the sts fall, −0.0078 against −0.0809, where the family gives a hundredth), and this is "the one place the table argues against the mechanism"; and ΦR = rtr = −½ ln(1 − r₁²q²) on the family falls with a fall in *either* factor, so the r₁-dominance result does not carry over to ΦR (Luppi et al., 2024, 2026). Among the studies in the table, Luppi et al. (2022) tested an autocorrelation-preserving null as just described; none of the others addressed autocorrelation, and Varley (2024) is the study whose subject it is.
...
**(a) What it says.** One dataset, one drug, one parcellation, N = 14; effect sizes with CIs; the DMT contrast is a proof of concept. The AR(1) family does not fit the data beyond lag 1 and its excess identity has the wrong sign on them; the map's q-axis and the diagnostic's substitution are AR(1) statements; the coupled family is symmetric; of the residual DiD, a third to two-thirds is finite-sample under one stationary null and the rest is not distinguishable from it, while the run-level residual on `ts_gsr` exceeds that null. There is no ground truth for synergy in these data. Two redundancy functions, both Gaussian and continuous; the discrete CCS on binarised data used elsewhere was not examined; the CCS definition used is the published one. The windowed estimator's manufacture is not removed by the diagnostic, and the split-half rule was set without reference to the reliability ceiling, so that test decided nothing. An odd/even-*TR* split within windows was not used because adjacent TRs share most of their variance at r₁ ≈ 0.86; odd/even *windows* is the right design and is underpowered here (about 20 effective samples per window, N = 14). Two computations earlier drafts listed as not run were run in the revision after the companion with their predictions recorded first: the regional test of the spatial-map claim (predicted branch obtained) and the run-level cross-lag deviation, whose statistic and reading were corrected twice afterwards (History; the superseded values in S9 Table). Its current reading rests on the budget of Results 4 under rules recorded before the run — finite sampling a minor part, the within-window term most, pooling a minor part and larger on the DMT run — which does not say what acts inside the windows (common slow drive and lagged coupling are one account at τ = 1); its null is the finite-sample null as specified, with a symmetric q distribution where 54.5 % of the data's pairs have q < 0, and the statistic on `ts_demean` (−0.0026) is reported and not read. Many statistics are reported without correction. The pre-specification history is as stated: the confirmatory test was of a different hypothesis, its directional-failure rule was written after the 115-region decrease was known, the Part B plans were written after the review computations; of the additions made after the second review, two had rules recorded on the day they were run, the coupled family was entered with no prediction, and the finite-sample null is a review computation with no rule. The git record establishes ordering, not blindness.

**(b) Why it is there.** It is the list a hostile reviewer would write, written first by the authors.

**(c) What a sceptic asks.** *You ran the two computations you had listed as not run — did they change anything?* Yes. The regional test made the spatial-map exposure a demonstrated fact on this dataset (Results 2). The cross-lag deviation was first read as removing the only named explanation of the run-level residual; a fifth review showed that the signed mean could not support that reading, and the sign(q)-weighted mean showed that explanation's signature; two computations of 16 September were then withdrawn by a correction note, and the exact budget that replaced them put most of the departure inside 2-minute windows, with pooling a minor part, larger on the DMT run, and what acts inside the windows open (Results 4). *"There is no ground truth for synergy in these data" — then what would count as evidence that any of this matters for the brain?* The paper's answer is that it is not trying to: "the analysis establishes what the estimator responds to, not what the brain does".
...
**(a) What it says.** S1 Text: the original pre-specified analysis and its results — the up-regulation hypothesis, the directional-failure rule, the windowed test, the intensity-tracking criterion with its controls and the void verdict, the redundancy prediction, the exploratory regional analysis, the EEG Lempel-Ziv check, robustness across estimators, and the commit-by-commit audit trail; in `draft.md` (kept as a record), `supplementary.md` (S1–S8 Tables) and `prespecification_summary.md`. S2 Text: the ΦR exploration (every result post-hoc) and why it was closed: ΦR with full inference on the raw series; HRF deconvolution and its effect on every quantity; the deconvolved whole-brain ΦR increase and a regional test against a prediction recorded before it was run; and the reasons the increase supports no claim — a pre-injection baseline gap in the direction that creates it (deconvolved −0.0124, p = 0.014; raw −0.0042, p = 0.28), a placebo-run decline across the 14 windows (deconvolved slope −0.00115 per window, p = 0.025; raw −0.00036, p = 0.43), estimator disagreement, and a 56–64 % placebo share; the within-pre-injection placebo decline is present on the raw series (−0.0084, p = 0.039) as well as after deconvolution (−0.0120, p = 0.067). S3 Text: the full applicability table with its search record (`partB5_literature_v2.md`), the scope-map tables, the CCS definition check, the split-half tables, the leave-two-out table, the coupled-family tables, the lag tables, the diagnostic tables, the matched-null tables, and the cross-lag deviation and regional sts–r₁ tables of the revision. S4 Text: the COBIDAS reporting checklist, each item marked reported (with location), not applicable (with reason) or not reported (with what is missing).
...
**(c) What a sceptic asks.** *A whole-brain ΦR increase after deconvolution, in the direction of the workspace account — and you closed it. Why?* The S2 Text pointer lists the four reasons with numbers: the baseline gap, the placebo decline, estimator disagreement, and a 56–64 % placebo share; the raw series shows no increase (Results 3: +0.0007, p = 0.90). *"The redundancy prediction" — what was it?* The revised S1 Text pointer says: under a redundancy-dominance reading of an sts decrease, rtr should have risen with a mirrored time course; it did not — on `ts_gsr` rtr fell with sts, positive in 4 of 14 subjects — and the reading is unsupported on either variant.

---

### Ethics statement (in Materials and methods, its first subsection, since 18 September 2026)

**(a) What it says.** The original study was approved by a named UK research ethics committee and the Health Research Authority, conducted under the Declaration of Helsinki, ICH Good Clinical Practice and the NHS Research Governance Framework, sponsored by Imperial College London under a Home Office licence for Schedule 1 drugs, with written informed consent; this wording follows Timmermann et al. (2023), which gives no reference number (a [TK] to obtain from C.T.). The present work is a secondary analysis of anonymised, parcellated time series and motion values released by the data authors; no new data, no participant contacted. Since 18 September the last sentence is exact rather than a claim about identifiability: the quantities used carry no demographic, image or identifying field; the released ratings table carries subject codes (an S, a two-digit subject number and two letters), which this analysis does not use and which the repository's tracked files do not contain (removed 15 September 2026; record, "Data-governance note").
...
**(a) What it says.** The time series, ratings, motion and EEG regressor are from the data authors' GitHub/Zenodo release (upstream commit 77af7aa), cloned locally and not redistributed. Everything else — analysis code, the pinned environment, the results tables, the pre-specification record, the six adversarial reviews (three as files; the fourth the verification of the correction note; the fifth quoted in the record's entry of 15 September, 18:14 UTC; the sixth, of 17 September, with its checks) and Part B notes with result files, and the closed-form implementation — is at the paper's GitHub repository. The tables under `results/` and the Part B tables added from 15 September name the commit that produced them in their headers (with a `-dirty` tag where the tree had uncommitted changes); the other review and Part B outputs carry none, and a single full run of `run_all.sh` regenerates them at one commit, except the superseded runs it lists and, without the sandbox, the deconvolution items [TK: the run sentence]. `run_all.sh` regenerates the original analysis (sections 0–5) and, in a section added on 15 September, the review and Part B computations and the five figures. Each script was run individually as recorded; the section has not yet been executed end-to-end as one run (a [TK] to do before submission). The deconvolution items need a separate sandbox and are skipped without it. Seed 20261120 throughout.

**(b) Why it is there.** The paper's credibility rests on the record being inspectable; this section says where and how.

**(c) What a sceptic asks.** *You claim full reproducibility but have never run the pipeline end to end.* The paper says so and marks it [TK]; the tables under `results/` and the Part B tables added from 15 September carry the SHA they ran at in their headers; the other review and Part B outputs carry none, so their runs are traceable through the record's dated entries until the single full run regenerates them at one commit. *The repository history contains subject codes that were removed from tracked files — has the history been rewritten?* No, and the paper's Ethics statement now says what is the case: the codes are in the data authors' public source release (`intensity_ratings.mat`, which V.S. read from a fresh clone of `singlesp/DMT_NCT` on 18 September 2026), this analysis does not use them, and the repository's tracked files do not contain them (removed 15 September 2026). The repository is public and reachable without login, so its history carries nothing the source release does not; a rewrite would be hygiene, not a governance necessity, and the history is left as it is, with this note (record, "PLOS Computational Biology form", 18 September 2026).
...
**(b) Why it is there.** It is what makes the "under the direction of V.S." of Use of AI tools (Materials and methods) concrete: every intellectual role is assigned to V.S., and the data authors' roles are the data.

**(c) What a sceptic asks.** *Is the software really V.S.'s if an AI system wrote it?* Use of AI tools answers: produced with the assistance of an AI system under V.S.'s direction, every number checked against its source before commit, responsibility taken by V.S.

---

### Funding (added 18 September 2026)

**(a) What it says.** V.S. received no specific funding for this work; the funding statements of C.T. and S.P.S., if any apply to their contributions, are a [TK].

**(b) Why it is there.** PLOS requires a funding statement for every author; an unfunded independent researcher says so.

**(c) What a sceptic asks.** *No funding, no institution — who paid for the compute and the AI system?* The paper does not say; the Funding statement covers research grants and institutional support, and the question is V.S.'s to answer.

---

### Competing interests ("Conflicts of interest" until 18 September 2026)
...
### Use of AI tools (in Materials and methods since 18 September 2026; "AI-use statement" in the back matter before that)

**(a) What it says.** Worded to PLOS's policy on AI tools since 18 September 2026, in four parts. The tool: an AI system, Claude (Anthropic), under V.S.'s direction, the model of each contribution identified with its session in the commit trailers (several models were used across the sessions, which is why no single model is named). How it was used: it wrote the analysis and verification code and the closed-form implementation, ran the review and Part B computations (rules and outcomes in the record's dated entries), drafted and revised the paper, the supplement and the notes to V.S.'s commissions, and conducted the six adversarial reviews. How its outputs were evaluated: every number checked against its source file before commit; the first three reviews re-traced every number of the draft they read and the fourth every number the revision had introduced, findings and corrections in `notes/` and the record; the fifth review's one finding quoted in the record; the sixth re-traced about 200 numbers, with its checks in `notes/fresh_review_2026-09-17/`. What it affected: the code, the computations, the reviews and the wording. The hypotheses, the pre-specification and the record's rules, the decisions on what was run, closed and reported, the interpretations, results, conclusions and limitations are the authors' own, and the authors are responsible for the accuracy and validity of the content and for the citation of sources. The [TK: journal-specific wording] is gone.
...
**(c) What a sceptic asks.** *Six "adversarial reviews" by the same AI that wrote the paper — is that adversarial?* The paper does not argue the point; it states what the reviews did (re-traced every number, found the errors listed in `notes/`, wrote and ran the finite-sample null, found the split-half flaw, found the dropped proportionality cell) and where their findings are. *What did you do?* CRediT and this statement together: direction, the record's rules, the checking of every number, and responsibility.

---
## Where explaining it exposed a problem

Every place where writing the plain-language version showed the paper stating something without justifying it, using a term it never defines, compressing a step a reader needs, or making a claim I could not rebuild from the paper's own text. Listed in the order of the paper, with the location first. Where I quote my own arithmetic it is labelled as mine. Every item was addressed in the paper's revision of 15 September 2026 (record, "Text revision after the plain-language companion"; items 29(a) and 29(b) by computations run under a pre-run entry, the rest by text); the list is kept as the record of what was found, with the table numbers of the revised paper. A fifth review then found that some of these had been addressed incompletely — items 3, 14, 15, 16, 21, 22, 29(a), 32 and 33 — and that the revision's own additions carried new problems, the largest being that the run-level cross-lag deviation of item 29(a), computed as a signed mean, could not test the mechanism it was run for; the revision after that review completed them (record, "Text revision after the fifth review"), and the units above describe the paper as it now stands.
