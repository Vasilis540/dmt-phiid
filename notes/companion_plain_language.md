...
**(c) What a sceptic asks.** *If r₁ explains 114 %, why is there a "remainder" at all?* Because 114 % is an over-prediction, and the over-prediction (the residual DiD, +0.0115) is itself significant against zero at p = 0.042 and not distinguishable from the null, every null value lying inside its interval; Results 4 ends its account of the residual with what "is not located": which stationary mechanism produces the run-level cross-lag deviation, and what produces the rest of the run-level residual and of the residual DiD. *Why recommend CCS if you also say CCS has no established reading?* The paper is careful: "consider CCS", and Discussion, "Redundancy functions", says "CCS is not offered as the corrected estimator, only as the one whose sts does not carry the autocorrelation contrast the way MMI-sts does".
