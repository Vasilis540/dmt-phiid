...
journal reader in the PLOS Computational Biology Methods-article order (main
text 9,991 words; title through Methods), with its supporting information as
five files under `manuscript/si/` and its tables in
`manuscript/supplementary.md`. B16b and B17b were run on 21 Sep 2026 and their
values are in the text; the [TK] marks left are the co-author items and the
sentence that will record the final full run.
...
| `manuscript/si/` | the supporting texts of `draft_v2.md`: `S1_Text.md` (the original pre-specified analysis), `S2_Text.md` (the ΦR exploration and deconvolution), `S3_Text.md` (the supporting tables of the estimator account), `S4_Text.md` (the COBIDAS pointer), `S5_Text.md` (provenance and audit trail) |
| `manuscript/supplementary.md` | supplementary tables S1–S11 (companion to `draft_v2.md`), every value quoted from `results/` and `notes/review_results/` |
| `manuscript/supplementary_cobidas.md` | the COBIDAS reporting checklist for `draft_v2.md` (reported / not applicable / not reported, with locations), pointed to by S4 Text |
| `manuscript/figures/` | `fig1_v2`–`fig5_v2` (pdf, png) and `captions_v2.md`, generated by `scripts/15_figures_v2.py`; the committed files are those of 48ea934 until the final full run, at which the revised script writes them and adds `fig1_v2_atoms_observed_predicted` and `fig6_v2_regional`; the `draft.md` figures and `captions.md` (from `scripts/12_figures.py`) are kept in place |
