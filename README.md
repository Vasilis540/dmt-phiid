...
text 9,994 words; title through Methods), with its supporting information as
five files under `manuscript/si/` and its tables in
`manuscript/supplementary.md`. Values that depend on the two computations of
21 Sep that have not yet been run (B16b, B17b) are marked `[TK: B16b]` and
`[TK: B17b]` and are filled from their committed tables in a follow-up commit.
