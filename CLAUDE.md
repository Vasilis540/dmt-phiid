...
## Current state (23 Sep 2026, round 16, Stage B: the shortened text, the outcomes of B21–B24, the figures regenerated)

**Analysis complete. The paper is `manuscript/draft_v2.md`, restructured on 21 Sep 2026 (round 14, Stage B) for a journal reader in the PLOS Computational Biology Methods-article order and shortened on 23 Sep 2026 (round 16, Stage B; record, "Stage B of round 16: the shortened text (23 Sep 2026)"): title "The Gaussian-MMI synergy atom of integrated information decomposition is mostly self-prediction: a closed form on the AR(1) family and a within-subject DMT fMRI application", abstract 300 and Author summary 196 words, Introduction through Methods 6,984 words with headings, three tables and six figures, the supporting information as `manuscript/si/S1_Text.md`–`S5_Text.md` and the tables S1–S18 in `manuscript/supplementary.md` (S12–S16 the former main-text Tables 4, 5, 6, 3 and 8; S17 every interval of a mean over subjects by method; S18 B23 and B24), and every number of the main text with its source in `manuscript/main_text_numbers.csv`. Every interval of a mean over subjects is B21's inverted sign-flip interval. `manuscript/draft.md` is the superseded first draft, kept as a record; `notes/companion_plain_language.md` is retired. The six figures `fig1_v2`–`fig6_v2` were regenerated by `scripts/15_figures_v2.py` at the text commit of bundle 27 and committed with their captions in the next commit (the captions file names the commit).**
...
- Remaining work: the single end-to-end run of `run_all.sh` at the final commit, under a new pre-run entry written
  ...
  names it; its outcome entry follows. The final run's `inference_revision.csv` will differ from 90690f4's only by
  the `np.float64` wrappers (the B21 outcome entry). The thirteen permanent [TK] items in `draft_v2.md` (affiliations,
  co-authors, the REC reference number, the co-authors' funding statements, contributions, competing interests, the
  run sentence) and the acknowledgments wait for the co-authors. Venue chosen (18 Sep 2026): PLOS Computational
  Biology, Methods article type; preprint to bioRxiv (`notes/venue_options.md`); the numbered citation form and the
  packaging of the Supporting information items as separate files are done at submission. Then a fresh review of the
...
manuscript/draft_v2.md           the paper (restructured 21 Sep 2026, shortened 23 Sep 2026)
...
manuscript/supplementary.md      supplementary tables S1–S18, values quoted from results/
                                 and notes/review_results/
manuscript/supplementary_cobidas.md  the COBIDAS reporting checklist, pointed to by S4 Text
manuscript/figures/              fig1_v2–fig6_v2 (pdf/png) + captions_v2.md, from 15_figures_v2.py
                                 (the captions file names the commit it ran at; fig1_v2_atoms_mmi_ccs
                                 is an earlier version's Figure 1, kept in place);
