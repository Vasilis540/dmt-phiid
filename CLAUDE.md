...
- Remaining work: the [TK] items in `draft_v2.md` (affiliations, co-authors,
  the REC reference number, journal wording, and the figures' git SHA: the
  five figures were generated at `git nogit` and are regenerated by
  `run_all.sh` section 6 in a git checkout); run `run_all.sh` end-to-end
  once before submission (started 15 Sep 2026, 10:25 UTC; in section 2 at
  15:40 UTC); choose the venue and preprint server
