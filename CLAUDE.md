...
## Current state (round 18, bundle 29: the error-only check of 25 September 2026 applied; the final run of `run_all.sh` next)

**Analysis complete. The paper is `manuscript/draft_v2.md`, restructured on 21 Sep 2026 (round 14, Stage B) for a journal reader in the PLOS Computational Biology Methods-article order, shortened on 23 Sep 2026 (round 16, Stage B), revised on 25 Sep 2026 (round 17) to every confirmed finding of the adversarial review, the citation check and their verification of 24 Sep 2026 (`notes/review_2026-09-24/`; record, "The review and the citation check of 24 September 2026, and their verification" and "Round 17: the review of 24 September 2026 applied (25 Sep 2026)"), and corrected in round 18 (bundle 29) to the error-only check of 25 Sep 2026 and its verification (`notes/review_2026-09-25/`; record, "The error-only check of 25 September 2026 and its verification" and "The error-only revision: the check of 25 September 2026 applied"): title "The Gaussian-MMI synergy atom of integrated information decomposition is mostly self-prediction: a closed form on the AR(1) family and a within-subject DMT fMRI application", abstract 300 words and Author summary 200 words, Introduction through Methods 6,984 words with headings (6,864 without), three tables and six figures, each table and figure caption in the text after the paragraph of its first citation; the supporting information as `manuscript/si/S1_Text.md`–`S5_Text.md` (S4 Text is now the COBIDAS checklist itself; `supplementary_cobidas.md` is gone) and the tables S1–S20 in `manuscript/supplementary.md` (S19 every prediction recorded before a computation with its outcome — of 52, 25 met, 13 partly met, 14 missed, 0 not evaluable — and the post hoc computations the paper quotes; S20 the applicability table); every number of the main text with its source in `manuscript/main_text_numbers.csv` (1,191 rows). The figures are numbered by order of first citation — Fig 1 the scope map, Fig 2 the atoms, Fig 3 the per-subject DiDs, Fig 4 the regional map, Fig 5 the AR(1)-substituted estimate by window, Fig 6 the lag dependence (`fig1_v2_scope_map` to `fig6_v2_lag_dependence`) — regenerated by `scripts/15_figures_v2.py` at the text commit of bundle 29 and committed with their captions in the next commit (the captions file names the commit). Notation: regional r₁, whole-brain r₁ and pair r₁ ("pair-level a" is withdrawn); the main text carries no computation label (B1–B24) and no file path outside Data and code availability, and the manuscript files carry no process label (round, stage or session names), dated descriptions replacing them, defined once in S5 Text §5. Every interval of a mean over subjects is B21's inverted sign-flip interval; the ratios of group means carry Fieller intervals (`notes/review_2026-09-24/checks/derived_r17.out`). `manuscript/draft.md` is the superseded first draft, kept as a record; `notes/companion_plain_language.md` is retired.**
...
- 23 Sep 2026, round 16, Stage B (bundle 27; record, the outcome entries of B21–B24 and "Stage B of round 16: the
  shortened text"): V.S. ran B21–B24 at a9d9ca4 (`runb21`, 8 min, every check passed; outputs committed as
  90690f4). The main text was shortened to 6,984 words (Introduction through Methods, headings counted) with three
  tables and six figures, material moving to the supporting information only; "confirmatory" became "primary"
  (Finding 6); every interval of a mean over subjects is B21's inverted interval (S17 Table lists all 932); the
  direction argument's withdrawal is carried into Results 4, which states the residual's response to each
  construction of a change in lagged structure (B17, B23; Table 3) and the aligned statistic in form (b) (B22 against
  B24's expectations); no estimator is preferred (Finding 9); B22 (d) replaces the per-SD ratio, B23 (c) the CCS
  statement, B23 (d) the "most exposed" statement and B23 (f) the unequal-coefficient statements. The figure script
  was revised (Figure 1 renamed `fig1_v2_atoms_observed_substituted`; Figure 2c's mirrored q = −0.25 curve; Figure 3
  redrawn with the OLS fits, the generators' rates and the cross-half relation; Figure 6c the residual map's network
  means with their spin test; inverted intervals as whiskers) and run in the writer's session, the one script the
  round allowed. `notes/partB21_inference_revision.py`'s CSV writer now formats `float(v)` (no value changes; the
  committed CSV is not edited). The session's two copies of `sch116_to_yeo.csv` are kept (V.S.'s decision).
- 25 Sep 2026, round 17 (bundle 28; record, the two entries named above): the adversarial review of the text at 66c6331 (15 major items, 44 minor, 6 typos), the citation check (111 sentence × work pairs: 88 supported, 6 with qualification, 4 not supported, 13 not checkable locally) and the planning session's verification of both, all of 24 Sep, committed byte-identical in `notes/review_2026-09-24/` with the reviewers' check scripts and `checks/derived_r17.py` (its output `derived_r17.out`: the Fieller intervals of the two ratios of group means, the leave-one-out cross-half table, the t intervals of the partialled contrast, the cortex-only regional correlation, the band-limit bound, the null's own pair-r₁ DiD; post hoc, no pre-run entry). V.S.'s decision applied every disposition of the verification: the abstract and author summary rewritten to the evidence (both per-SD ratios; the like-for-like comparison with the band-passed generator; the disattenuated correlation with its interval; the refutation scoped to MMI-sts); the notation of r₁ fixed; computation labels, file paths and process labels removed from the main text and the SI; the finite-sample null described as carrying the data's autocorrelation change; the residual's rate read against the generators by the data's own intervals; Results 4's construction-by-construction paragraphs and the directed part moved to S3 Text §6; the Limitations sentence of 90690f4 restored; the Ethics statement "pseudonymised", with the subject-code sentence restored in S1 Text; the AI-use statement rewritten to V.S.'s facts (who ran what, and the data-copy disclosure of S5 Text §4); S19 and S20 Tables added; S4 Text made the COBIDAS checklist itself; S5 Text corrected (E1–E8, P9, the -dirty/nogit inventory, the reference-list head note, the review of 24 Sep); S2 Text given the deconvolution's parameters; the figures renumbered by order of first citation and regenerated in the writer's session, the one script the round allowed (Fig 1a's contour labels moved, Fig 2's annotation to the unequal-coefficient finding, Fig 3's caption with the 6 % scale note, Fig 5c's expectation band visible with its generator named); `main_text_numbers.csv` rebuilt for the revised text (1,183 rows, 0 failures). Two corrections to the entries of 23 Sep 2026 21:40 UTC are recorded (the cause of B21's seven exact-p differences; who set the section budgets).
- Round 18 (bundle 29; record, "The error-only check of 25 September 2026 and its verification", "The error-only revision: the check of 25 September 2026 applied" and the final run's pre-run entry): a check of the text at ddae618 restricted to outright errors, by a separate session (41 findings: 10 text, 4 numbers, 10 cross-references, 10 contradictions, 7 uncertain), and the planning session's end-to-end read, both verified against the committed files (`notes/review_2026-09-25/verification_2026-09-25.md`: 40 confirmed, 1 partly; the read's V1–V24; V25–V61 found in preparing the revision and the final run, V35–V52 by an audit of the prepared revision by two further sessions and V55–V61 by a second audit, of the corrections, by a third), applied by 131 verbatim replacements (`notes/review_2026-09-25/revision/`: 26 in `draft_v2.md`; S1–S5 Text; 36 in `supplementary.md`; 24 in the figure script) including the minus sign of 583 and 300 negative numbers in S3 Text and `supplementary.md`; `main_text_numbers.csv` rebuilt (1,191 rows); the figures regenerated (Figs 2–6 changed; Fig 3's index labels placed clear of each other; Fig 5c's legend below the axis); `15_figures_v2.py` given `rev_git.py`'s SHA rule; `6_committed_compare.py` and `8_binary_compare.py` revised and `9_wrapper_compare.py` and `10_logs_figures_compare.py` added for the final run; Introduction through Methods 6,984 words with headings.
- Remaining work: the single end-to-end run of `run_all.sh` at the commit that carries its pre-run entry (record, "The
  final end-to-end run of `run_all.sh` at the final commit: pre-run entry") — V.S., after a reboot with the lid switch
  set to `ignore`, as a root transient unit that runs it as his user (`runuser -u vilalius`) inside `systemd-inhibit
  --what=sleep:idle:handle-lid-switch:shutdown --mode=block`, with the charger connected and the lid open for the
  whole run (four attempts on 16–17 Sep stopped inside `02_bias_check`, two of them within minutes of the lid closing;
  record, "The attempts at the end-to-end run"; two more at 0b8d1a4 on 21–23 Sep were ended by a power-off from the
  desktop session; record, the entry closing the pre-run entry of 21 Sep). Its pass rule: `6_committed_compare.py`
  (revised on 25 Sep to compare the text files line by line, in order, with every printed number within 1e-9),
  `8_binary_compare.py` (revised on 25 Sep to compare the list-of-dict pickles element by element),
  `9_wrapper_compare.py` (B21's two files: the `np.float64` wrappers and the text-dependent `quoted_at`, non-empty in
  10 rows against 202 at 90690f4) and `10_logs_figures_compare.py` (the logs, every non-blank committed line
  reproduced in order under masks for what changes between runs, the added lines listed; and every tracked image), all
  on the working tree before anything is touched, with the differences known in advance listed in the entry (the 84
  and 2 AR-shift rows of the non-stationary bias tables; the hand-added line of the subject-alignment report and its
  log, and the log's empty line 86; two lines of the two global-fit logs of section 2). Then bundle 30: the outputs,
  the new files the run writes, the run's log and heartbeat as `results/run_all_final.log` and
  `results/run_all_final_heartbeat.log`, the outcome entry, the run's [TK]s filled within their lines (Data and code
  availability; S5 Text §4) and S5 Text §6's list of commits; then the typeset PDF and the note to C.T. The [TK] items
  in `draft_v2.md` (affiliations, co-authors, the REC reference number, the co-authors' funding statements,
  contributions, competing interests) and the acknowledgments wait for the co-authors; the REC reference number and
  the basis on which the approval or the release covers secondary use come from C.T. Venue chosen (18 Sep 2026): PLOS
  Computational Biology, Methods article type; preprint to bioRxiv (`notes/venue_options.md`). At submission: remove
  the "Manuscript for co-author review; not for citation or distribution." line from Data and code availability; move
  Funding, Competing interests, Data and code availability and Author contributions from the file to the submission
  system's fields (they stay in the file for the preprint); convert the author–year citations to the numbered form;
  package the Supporting information items (S1–S5 Text, S1–S20 Table) as separate files; archive the repository with a
  DOI and cite it in Data and code availability.
...
manuscript/draft_v2.md           the paper (restructured 21 Sep 2026, shortened 23 Sep 2026, revised 25 Sep 2026)
manuscript/main_text_numbers.csv every number of the main text with its source file and line
...
manuscript/si/                   S1_Text.md–S5_Text.md, the supporting texts of draft_v2.md
                                 (S4_Text.md is the COBIDAS reporting checklist)
manuscript/supplementary.md      supplementary tables S1–S20, values quoted from results/,
                                 notes/review_results/ and the record (S19, S20)
manuscript/figures/              fig1_v2_scope_map, fig2_v2_atoms_observed_substituted,
                                 fig3_v2_per_subject, fig4_v2_regional, fig5_v2_residual_diagnostic,
                                 fig6_v2_lag_dependence (pdf/png) + captions_v2.md, from
                                 15_figures_v2.py, numbered by order of first citation since 25 Sep
                                 2026 (the captions file names the commit it ran at;
                                 fig1_v2_atoms_mmi_ccs is an earlier version's Figure 1, kept in place);
...
notes/                           adversarial reviews, the verification of the correction note
                                 ...
                                 submission of 20 Sep (review_2026-09-20/), the review and citation
                                 pass of 22 Sep with their verification (review_2026-09-22/), the
                                 review and citation check of 24 Sep with their verification, the
                                 check scripts and derived_r17.out (review_2026-09-24/), the error-only check of 25 Sep with its
                                 verification, the revision's replacements and the check scripts
                                 (review_2026-09-25/), the applicability
                                 ...
                                 session's checks and V.S.'s reproduction checks (reproduction_checks/:
                                 6_committed_compare.py, 8_binary_compare.py, 9_wrapper_compare.py and
                                 10_logs_figures_compare.py, the four comparisons of the final run); the regional
